...
static const int NUM_MULTISCALE = 4;
...
template<typename BitChance> class MultiscaleBitChanceTable {
public:
    typedef typename BitChance::Table SubTable;
    SubTable subTable[NUM_MULTISCALE];

    MultiscaleBitChanceTable(int cut = 8) {
        for (int i= 0; i<NUM_MULTISCALE; i++) {
            subTable[i].init(cut, MULTISCALE_ALPHAS[i]);
        }
    }
};

template<typename BitChance> class MultiscaleBitChance
{
protected:
    BitChance chances[NUM_MULTISCALE];
    uint32_t quality[NUM_MULTISCALE];
    int best;

public:
    typedef MultiscaleBitChanceTable<BitChance> Table;
    ...
    void put(bool bit, const Table &table) {
        for (int i=0; i<NUM_MULTISCALE; i++) { // for each scale
            uint64_t sbits = 0;
